...
def create_app() -> FastAPI:
    ...
    @app.get("/api/manual/cluster-load")
    ...
    @app.get("/api/manual/disk-remaining")
        ...
        <div class="row mb-3">
          <div class="col-12">
            <div class="main-card">
              <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title mb-0">
                  <i class="bi bi-terminal-fill me-2" style="color:#17a2b8;"></i>手动指标查询
                </h5>
              </div>
              <div class="card-body">
                <div class="row g-3">
                  <div class="col-md-3">
                    <button class="btn btn-outline-primary w-100" onclick="uiManager.dataManager.queryClusterLoad()">查询集群负载</button>
                    <div id="manual-cluster-load" class="mt-2"></div>
                  </div>
                  <div class="col-md-3">
                    <div class="input-group">
                      <span class="input-group-text">节点(instance)</span>
                      <input id="node-input" class="form-control" placeholder="192.168.x.x:9100" value="192.168.4.108:9100" />
                    </div>
                    <button class="btn btn-outline-secondary w-100 mt-2" onclick="uiManager.dataManager.queryNodeLoad(document.getElementById('node-input').value)">查询节点负载</button>
                    <div id="manual-node-load" class="mt-2"></div>
                  </div>
                  <div class="col-md-3">
                    <div class="input-group">
                      <span class="input-group-text">命名空间</span>
                      <input id="ns-input" class="form-control" placeholder="default" />
                    </div>
                    <div class="input-group mt-2">
                      <span class="input-group-text">Pod</span>
                      <input id="pod-input" class="form-control" placeholder="pod-name" />
                    </div>
                    <button class="btn btn-outline-secondary w-100 mt-2" onclick="uiManager.dataManager.queryPodLoad(document.getElementById('ns-input').value, document.getElementById('pod-input').value)">查询Pod负载</button>
                    <div id="manual-pod-load" class="mt-2"></div>
                  </div>
                  <div class="col-md-3">
                    <div class="input-group">
                      <span class="input-group-text">节点(instance)</span>
                      <input id="disk-node-input" class="form-control" placeholder="192.168.x.x:9100" value="192.168.4.108:9100" />
                    </div>
                    <div class="input-group mt-2">
                      <span class="input-group-text">挂载点</span>
                      <input id="mp-input" class="form-control" value="/" />
                    </div>
                    <button class="btn btn-outline-secondary w-100 mt-2" onclick="uiManager.dataManager.queryDiskRemaining(document.getElementById('disk-node-input').value, document.getElementById('mp-input').value)">查询磁盘剩余</button>
                    <div id="manual-disk-remaining" class="mt-2"></div>
                  </div>
                </div>
              </div>
            </div>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>巡检报告 - AI-Ops 智能运维系统</title>
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="navbar-nav ms-auto">
                ...
                <a class="nav-link active" href="/reports">
                    <i class="bi bi-file-earmark-text me-1"></i>巡检报告
                </a>
                <a class="nav-link" href="/server-resources">
                    <i class="bi bi-server me-1"></i>服务器资源
                </a>
            </div>
        </div>
    </nav>
    ...
    <div class="container-fluid py-4">
        <div class="page-header">
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    <h1 class="page-title">
                        <i class="bi bi-file-earmark-text me-3"></i>巡检报告中心
                    </h1>
                    <p class="page-subtitle">全面分析巡检数据，洞察系统健康状况，生成专业报告</p>
                </div>
            ...
            <div class="filter-row">
                <div class="filter-group">
                    ...
                    <div class="time-picker">
                        <button class="btn btn-outline-primary" data-range="1d">1天</button>
                        <button class="btn btn-outline-primary" data-range="3d">3天</button>
                        <button class="btn btn-outline-primary active" data-range="7d">7天</button>
                        <button class="btn btn-outline-primary" data-range="30d">30天</button>
                    </div>
                </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-value" id="total-inspections">--</div>
                <div class="stat-label">总巡检次数</div>
            </div>
            <div class="stat-card success">
                <div class="stat-value" id="avg-health-score">--</div>
                <div class="stat-label">平均健康评分</div>
            </div>
            <div class="stat-card warning">
                <div class="stat-value" id="min-health-score">--</div>
                <div class="stat-label">最低健康评分</div>
            </div>
            <div class="stat-card info">
                <div class="stat-value" id="max-health-score">--</div>
                <div class="stat-label">最高健康评分</div>
            </div>
        </div>
        ...
        <div class="row mb-4">
            <div class="col-lg-6 mb-3">
                <div class="main-card">
                    <div class="card-header">
                        <h5>
                            <i class="bi bi-graph-up me-2 text-primary"></i>健康趋势分析
                        </h5>
                        <div class="header-actions">
                            <small class="text-muted">最近7天健康评分变化</small>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="chart-container">
            ...
            <div class="col-lg-6 mb-3">
                <div class="main-card">
                    <div class="card-header">
                        <h5>
                            <i class="bi bi-pie-chart me-2 text-primary"></i>状态分布
                        </h5>
                        <div class="header-actions">
                            <small class="text-muted">检查结果状态统计</small>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="chart-container">
        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="main-card">
                    <div class="card-header">
                        <h5>
                            <i class="bi bi-bar-chart me-2 text-primary"></i>类别分布分析
                        </h5>
                        <div class="header-actions">
                            <small class="text-muted">各类别检查数量统计</small>
                        </div>
                    </div>
                    <div class="card-body">
        ...
        <div class="main-card">
            <div class="card-header">
                <h5>
                    <i class="bi bi-list-ul me-2 text-primary"></i>数据摘要
                </h5>
            </div>
            <div class="card-body">
                <div id="summary-info">
        ...
        <div class="main-card">
            <div class="card-header">
                <h5>
                    <i class="bi bi-table me-2 text-primary"></i>巡检详细记录
                </h5>
                <div class="header-actions">
                    <small class="text-muted" id="table-info">共 -- 条记录</small>
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
    ...
    <div id="loading-overlay" class="position-fixed top-0 start-0 w-100 h-100 d-none" 
         style="background: rgba(0,0,0,0.5); z-index: 9999; align-items: center; justify-content: center;">
    ...
    def server_resources_page() -> str:
        """服务器资源监控页面 - 浅色主题（与首页/详情页一致，可搜索、排序、分页、指标卡）"""
        return """
<!DOCTYPE html>
<html lang=\"zh-CN\">
<head>
    ...
    <title>服务器资源监控 - AI-Ops 巡检系统</title>
    <script src=\"/static/js/axios.min.js\"></script>
    <link href=\"/static/css/bootstrap.min.css\" rel=\"stylesheet\" />
    <link href=\"/static/css/bootstrap-icons.css\" rel=\"stylesheet\" />
    <style>
      :root { --primary-gradient: linear-gradient(135deg, #667eea 0%, #764ba2 100%); --card-shadow: 0 8px 32px rgba(0,0,0,.08); --card-hover-shadow: 0 12px 40px rgba(0,0,0,.12); }
      body { background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); color: #212529; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
      .navbar { background: var(--primary-gradient) !important; box-shadow: 0 4px 20px rgba(0,0,0,.1); }
      .toolbar { background: #ffffff; border: 1px solid #dee2e6; border-radius: 16px; padding: 14px; box-shadow: var(--card-shadow); position: sticky; top: 12px; z-index: 50; }
      .toolbar .form-control, .toolbar .form-select { background-color: #ffffff; border-color: #dee2e6; color: #212529; }
      .toolbar .form-control::placeholder { color: #9aa3b2; }
      .chip { display: inline-flex; align-items: center; gap: 6px; border: 1px solid #dee2e6; background: #fff; color: #495057; padding: 8px 12px; border-radius: 999px; font-size: 13px; }
      .chip .dot { width: 8px; height: 8px; border-radius: 50%; display: inline-block; }
      .chip .dot.ok { background: #198754; }
      .chip .dot.warn { background: #ffc107; }
      .chip .dot.err { background: #dc3545; }
      .metric-card { border: none; background: #ffffff; color: #212529; border-radius: 16px; padding: 18px; height: 100%; box-shadow: var(--card-shadow); }
      .metric-title { color: #6c757d; font-size: 12px; letter-spacing: .5px; text-transform: uppercase; margin-bottom: 6px; }
      .metric-value { font-size: 22px; font-weight: 700; background: var(--primary-gradient); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
      .server-card { border: none; background: #ffffff; border-radius: 16px; overflow: hidden; transition: transform .18s ease, box-shadow .18s ease; height: 100%; box-shadow: var(--card-shadow); }
      .server-card:hover { transform: translateY(-2px); box-shadow: var(--card-hover-shadow); }
      .server-hd { display:flex; align-items:center; justify-content: space-between; padding: 12px 14px; border-bottom: 1px solid #dee2e6; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); }
      .server-hd h6 { margin: 0; font-weight: 600; font-size: 14px; color: #212529; }
      .server-meta { color: #6c757d; font-size: 12px; }
      .progress { height: 20px; background-color: #eef2f7; border-radius: 12px; }
      .progress .progress-bar { border-radius: 12px; }
      .btn-ghost { background: #ffffff; border: 1px solid #dee2e6; color: #495057; border-radius: 10px; padding: 6px 12px; }
      .btn-ghost:hover { background: #f8f9fa; }
      .pill { padding: 6px 10px; border-radius: 999px; border:1px solid #dee2e6; background:#ffffff; color:#6c757d; font-size:12px; }
      .footer-pager { border: 1px solid #dee2e6; background:#ffffff; border-radius: 12px; padding: 10px 12px; box-shadow: var(--card-shadow); }
      .muted { color:#6c757d; }
      .loading { text-align:center; color:#6c757d; padding: 28px; }
    </style>
</head>
<body>
  <nav class=\"navbar navbar-expand-lg navbar-dark\">
    <div class=\"container-fluid\">
      <a class=\"navbar-brand\" href=\"/\"><i class=\"bi bi-speedometer2\"></i> AI-Ops 巡检系统</a>
      <div class=\"navbar-nav ms-auto\">
        <a class=\"nav-link\" href=\"/\">仪表板</a>
        <a class=\"nav-link\" href=\"/log-types\">日志类型</a>
        <a class=\"nav-link active\" href=\"/server-resources\">服务器资源</a>
    ...
    <div class=\"toolbar mb-3\">
      <div class=\"row g-2 align-items-center\">
        <div class=\"col-md-3 col-sm-6\">
          <div class=\"input-group\">
            <span class=\"input-group-text bg-transparent text-muted border-0\"><i class=\"bi bi-search\"></i></span>
            <input id=\"sr-search\" type=\"text\" class=\"form-control\" placeholder=\"搜索实例或主机...\" />
          </div>
        </div>
        <div class=\"col-md-2 col-6\">
          <select id=\"sr-sort\" class=\"form-select\">
            <option value=\"max\" selected>排序：CPU/内存最大值</option>
            <option value=\"cpu\">排序：CPU使用率</option>
            <option value=\"mem\">排序：内存使用率</option>
            <option value=\"disk\">排序：磁盘使用率(最大分区)</option>
          </select>
        </div>
        <div class=\"col-md-2 col-6\">
          <select id=\"sr-page-size\" class=\"form-select\">
        ...
        <div class=\"col-md-3 d-none d-md-block\">
          <span id=\"sr-active-filter\" class=\"pill\" style=\"display:none;\"></span>
        </div>
        <div class=\"col-md-2 text-end\">
          <div class=\"d-flex justify-content-end align-items-center gap-2\">
            <div class=\"form-check form-switch\">
              <input class=\"form-check-input\" type=\"checkbox\" role=\"switch\" id=\"sr-auto\" checked />
              <label class=\"form-check-label muted\" for=\"sr-auto\">自动刷新</label>
            </div>
            <button id=\"sr-refresh\" class=\"btn btn-ghost\"><i class=\"bi bi-arrow-clockwise\"></i> 刷新</button>
          </div>
        </div>
      </div>
      <div class=\"row g-2 mt-2\" id=\"sr-metrics\"></div>
    </div>

    <div id=\"sr-grid\" class=\"row g-3\">
      <div class=\"loading\"><div class=\"spinner-border spinner-border-sm text-light\"></div><div class=\"mt-2\">加载中...</div></div>
    </div>

    <div class=\"d-flex justify-content-between align-items-center mt-3 footer-pager\" id=\"sr-pager\"></div>
  </div>

  <script>
    const SR_STATE = { items: [], filtered: [], page: 1, pageSize: 12, sortBy: 'max', sortDir: 'desc', query: '', auto: true, instanceFilter: null };
    ...
    function pickDiskMax(server){
      try { return Math.max(...(server.disk?.partitions||[]).map(p=>Number(p.usage_percent||0))); } catch(e){ return 0; }
    }
    function computeMetrics(list){
      const n = list.length || 0; if(!n) return {count:0, avgCpu:0, avgMem:0, high:0};
      let cpu=0, mem=0, high=0;
      list.forEach(s=>{ const c=Number(s.cpu?.usage_percent||0), m=Number(s.memory?.usage_percent||0); cpu+=c; mem+=m; if(Math.max(c,m)>=80) high++; });
      return {count:n, avgCpu: cpu/n, avgMem: mem/n, high};
    }
    function applyFilters(){
      let arr = SR_STATE.items.slice();
      if (SR_STATE.instanceFilter){ arr = arr.filter(s=> (s.instance||'').includes(SR_STATE.instanceFilter)); const pill = document.getElementById('sr-active-filter'); pill.textContent = '实例筛选: '+SR_STATE.instanceFilter; pill.style.display='inline-flex'; pill.onclick = ()=>{ SR_STATE.instanceFilter=null; pill.style.display='none'; applyFilters(); renderAll(); } }
      const q = SR_STATE.query.trim().toLowerCase();
      if (q){ arr = arr.filter(s=> (s.instance||'').toLowerCase().includes(q) || (s.system?.hostname||'').toLowerCase().includes(q)); }
      const sortKey = SR_STATE.sortBy;
      arr.sort((a,b)=>{
        const cpuA=Number(a.cpu?.usage_percent||0), cpuB=Number(b.cpu?.usage_percent||0);
        const memA=Number(a.memory?.usage_percent||0), memB=Number(b.memory?.usage_percent||0);
        const diskA=pickDiskMax(a), diskB=pickDiskMax(b);
        const keyA = sortKey==='cpu'?cpuA: sortKey==='mem'?memA: sortKey==='disk'?diskA: Math.max(cpuA, memA);
        const keyB = sortKey==='cpu'?cpuB: sortKey==='mem'?memB: sortKey==='disk'?diskB: Math.max(cpuB, memB);
    ...
    function renderMetrics(){
      const m = computeMetrics(SR_STATE.filtered);
      const el = document.getElementById('sr-metrics');
      el.innerHTML = `
        <div class=\"col-md-3 col-6\"><div class=\"metric-card\"><div class=\"metric-title\">服务器数量</div><div class=\"metric-value\">${m.count}</div></div></div>
        <div class=\"col-md-3 col-6\"><div class=\"metric-card\"><div class=\"metric-title\">平均CPU</div><div class=\"metric-value\">${m.avgCpu.toFixed(1)}%</div></div></div>
        <div class=\"col-md-3 col-6\"><div class=\"metric-card\"><div class=\"metric-title\">平均内存</div><div class=\"metric-value\">${m.avgMem.toFixed(1)}%</div></div></div>
        <div class=\"col-md-3 col-6\"><div class=\"metric-card\"><div class=\"metric-title\">高负载(>=80%)</div><div class=\"metric-value\">${m.high}</div></div></div>
      `;
    }
    function barClass(v){ return v>=80? 'bg-danger' : v>=60 ? 'bg-warning' : 'bg-success'; }
    function renderGrid(){
      const grid = document.getElementById('sr-grid');
      if (!SR_STATE.filtered.length){ grid.innerHTML = '<div class=\"loading\">暂无服务器资源信息</div>'; return; }
      ...
      pageItems.forEach(sv=>{
        ...
        const cpu = Number(sv.cpu?.usage_percent||0), mem = Number(sv.memory?.usage_percent||0);
        const disk = pickDiskMax(sv);
        const cores = sv.cpu?.cores || 0; const memGb = sv.memory?.total_gb || 0; const up = Number(sv.system?.uptime_days||0);
        html += `
        <div class=\"col-12 col-md-6 col-xl-4\">
          <div class=\"server-card\">
            <div class=\"server-hd\">
              <h6><i class=\"bi bi-hdd-network\"></i> ${instance}</h6>
              <span class=\"chip\"><span class=\"dot ${cpu>=80||mem>=80?'err': cpu>=60||mem>=60?'warn':'ok'}\"></span>${hostname}</span>
            </div>
            <div class=\"p-3\">
              <div class=\"mb-2\"><small class=\"muted\">CPU 使用率</small>
                <div class=\"progress\"><div class=\"progress-bar ${barClass(cpu)}\" style=\"width:${cpu}%\">${formatPct(cpu)}</div></div>
              </div>
              <div class=\"mb-2\"><small class=\"muted\">内存 使用率</small>
                <div class=\"progress\"><div class=\"progress-bar ${barClass(mem)}\" style=\"width:${mem}%\">${formatPct(mem)}</div></div>
              </div>
              <div class=\"mb-2\"><small class=\"muted\">磁盘 使用率(最大)</small>
                <div class=\"progress\"><div class=\"progress-bar ${barClass(disk)}\" style=\"width:${disk}%\">${formatPct(disk)}</div></div>
              </div>
              <div class=\"d-flex justify-content-between server-meta mt-2\">
                <span>核心: <strong>${cores}</strong></span>
                <span>内存: <strong>${memGb} GB</strong></span>
                <span>运行: <strong>${up.toFixed(1)} 天</strong></span>
              </div>
              <div class=\"mt-3 d-flex justify-content-end\">
                <a class=\"btn btn-ghost btn-sm\" href=\"/server-resources?instance=${encodeURIComponent(instance)}\"><i class=\"bi bi-aspect-ratio\"></i> 仅看此实例</a>
              </div>
            </div>
          </div>
        </div>`;
    ...
    function renderPager(){
      ...
      const prevDis = SR_STATE.page<=1 ? 'disabled' : '';
      const nextDis = SR_STATE.page>=totalPages ? 'disabled' : '';
      pager.innerHTML = `
        <div class=\"muted\">共 ${total} 台 · 每页 ${SR_STATE.pageSize} 台</div>
        <div class=\"d-flex align-items-center gap-2\">
          <button class=\"btn btn-ghost btn-sm ${prevDis}\" onclick=\"gotoPage(${SR_STATE.page-1})\">上一页</button>
          <span class=\"muted\">第 ${SR_STATE.page}/${totalPages} 页</span>
          <button class=\"btn btn-ghost btn-sm ${nextDis}\" onclick=\"gotoPage(${SR_STATE.page+1})\">下一页</button>
        </div>`;
    }
    function renderAll(){ renderMetrics(); renderGrid(); renderPager(); }
    function gotoPage(p){ if (p<1) return; SR_STATE.page = p; renderAll(); }
    ...
    async function fetchData(initial=false){
      try{
        if (initial){
          ...
          SR_STATE.items = Array.isArray(quick.data?.data) ? quick.data.data : (Array.isArray(quick.data) ? quick.data : []);
          applyFilters(); renderAll();
          // 后台强制刷新
          axios.get(`/api/server-resources?refresh=true&t=${Date.now()}`, {timeout: 60000}).then(r=>{
            const fresh = Array.isArray(r.data?.data) ? r.data.data : (Array.isArray(r.data) ? r.data : []);
            if (fresh && fresh.length){ SR_STATE.items = fresh; applyFilters(); renderAll(); }
          }).catch(()=>{});
          return;
        }
        const url = `/api/server-resources?t=${Date.now()}`;
        const resp = await axios.get(url, {timeout: 30000});
        SR_STATE.items = Array.isArray(resp.data?.data) ? resp.data.data : (Array.isArray(resp.data) ? resp.data : []);
        applyFilters(); renderAll();
      }catch(e){
        document.getElementById('sr-grid').innerHTML = `<div class=\"loading\"><i class=\"bi bi-exclamation-triangle\"></i> 加载失败：${e.message}</div>`;
      }
    }

    // events
    function attachEvents(){
      document.getElementById('sr-refresh').onclick = ()=> fetchData(true);
      document.getElementById('sr-auto').onchange = (e)=>{ SR_STATE.auto = !!e.target.checked; };
      document.getElementById('sr-sort').onchange = (e)=>{ SR_STATE.sortBy = e.target.value; SR_STATE.page=1; applyFilters(); renderAll(); };
      document.getElementById('sr-page-size').onchange = (e)=>{ SR_STATE.pageSize = parseInt(e.target.value||'12',10); SR_STATE.page=1; applyFilters(); renderAll(); };
      const search = document.getElementById('sr-search');
      let t=null; search.oninput = (e)=>{ clearTimeout(t); t=setTimeout(()=>{ SR_STATE.query = e.target.value||''; SR_STATE.page=1; applyFilters(); renderAll(); }, 200); };
    }
    function tick(){ if (!SR_STATE.auto) return; fetchData(false); }

    document.addEventListener('DOMContentLoaded', ()=>{
      readQuery(); attachEvents(); fetchData(true); setInterval(tick, 30000);
    });
  </script>
</body>
</html>
...
<head>
  ...
  <style>
    ...
    .navbar { background: var(--primary-gradient)!important; box-shadow: 0 4px 20px rgba(0,0,0,.1); }
    .main-card { background: #fff; border: none; border-radius: 20px; box-shadow: var(--card-shadow); transition: all .3s ease; }
    .main-card:hover { box-shadow: var(--card-hover-shadow); }
    .main-card .card-header { background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-bottom: 1px solid #dee2e6; border-radius: 20px 20px 0 0!important; padding: 1.25rem 1.5rem; }
    .metric-card { background:#fff; border: none; border-radius: 20px; box-shadow: var(--card-shadow); overflow: hidden; position: relative; }
    .metric-card::before { content:''; position:absolute; top:0; left:0; right:0; height:4px; background: var(--primary-gradient); }
    .metric-card .card-body { padding: 1.25rem 1.25rem; }
    .metric-title { color:#6c757d; font-size: .9rem; text-transform: uppercase; letter-spacing: .5px; }
    .metric-value { font-size: 2rem; font-weight: 700; background: var(--primary-gradient); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
    .progress { height: 20px; border-radius: 12px; overflow: hidden; background: #eef2f7; }
    .badge-soft { background: #eef2ff; color: #4e5ae8; }
    .kv { display:flex; justify-content: space-between; padding: .35rem 0; border-bottom: 1px dashed #e9ecef; }
  ...
  <script>
    ...
    function pickDiskMax(s){ try { return Math.max(...(s.disk?.partitions||[]).map(p=>Number(p.usage_percent||0))); } catch(e){ return 0; } }
    async function load(){
      const instance = q('instance');
      document.getElementById('title-inst').textContent = instance || '未知实例';
      try {
        // 快速读取缓存
        const quick = await axios.get('/api/server-resources?quick=true&t=' + Date.now());
        let sv = (quick.data?.data||quick.data||[]).find(s=> (s.instance||'') === instance);
        if (!sv) {
          // 后台强制刷新并重试
          try { await axios.get('/api/server-resources?refresh=true&t=' + Date.now(), {timeout: 60000}); } catch(e) {}
          const fresh = await axios.get('/api/server-resources?t=' + Date.now());
          sv = (fresh.data?.data||fresh.data||[]).find(s=> (s.instance||'') === instance);
        }
        render(sv, instance);
      } catch(e){
        document.getElementById('content').innerHTML = '<div class="text-center text-muted py-5">加载失败：' + (e.message||e) + '</div>';
      }
    }
    function colorBy(v){ return v>=80? 'bg-danger' : v>=60? 'bg-warning' : 'bg-success' }
    function render(sv, inst){
      if (!sv){
        document.getElementById('content').innerHTML = '<div class="text-center text-muted py-5">未找到该实例数据</div>';
        return;
      }
      const cpu = Number(sv.cpu?.usage_percent||0);
      const mem = Number(sv.memory?.usage_percent||0);
      const diskMax = pickDiskMax(sv);
      const cores = sv.cpu?.cores || 0;
      const memGb = sv.memory?.total_gb || 0;
      const up = Number(sv.system?.uptime_days||0);
      const hostname = (sv.system?.hostname) || (inst.split(':')[0]||'');

      // 顶部指标
      document.getElementById('top-metrics').innerHTML = `
        <div class="col-md-3 col-6"><div class="metric-card"><div class="card-body text-center"><div class="metric-title">CPU 使用率</div><div class="metric-value">${cpu.toFixed(1)}%</div></div></div></div>
        <div class="col-md-3 col-6"><div class="metric-card"><div class="card-body text-center"><div class="metric-title">内存 使用率</div><div class="metric-value">${mem.toFixed(1)}%</div></div></div></div>
        <div class="col-md-3 col-6"><div class="metric-card"><div class="card-body text-center"><div class="metric-title">磁盘(最大)</div><div class="metric-value">${diskMax.toFixed(1)}%</div></div></div></div>
        <div class="col-md-3 col-6"><div class="metric-card"><div class="card-body text-center"><div class="metric-title">运行天数</div><div class="metric-value">${up.toFixed(1)}</div></div></div></div>`;

      // 资源进度条
      document.getElementById('bars').innerHTML = `
        <div class="mb-3"><small class="text-muted">CPU 使用率</small><div class="progress mt-1"><div class="progress-bar ${colorBy(cpu)}" style="width:${cpu}%">${pct(cpu)}</div></div></div>
        <div class="mb-3"><small class="text-muted">内存 使用率</small><div class="progress mt-1"><div class="progress-bar ${colorBy(mem)}" style="width:${mem}%">${pct(mem)}</div></div></div>
        <div class="mb-1"><small class="text-muted">磁盘 使用率(最大分区)</small><div class="progress mt-1"><div class="progress-bar ${colorBy(diskMax)}" style="width:${diskMax}%">${pct(diskMax)}</div></div></div>`;

      // 基本信息
      document.getElementById('kv').innerHTML = `
        <div class="kv"><span class="text-muted">实例</span><span class="fw-semibold">${inst}</span></div>
                        <div class="kv"><span class="text-muted">主机名</span><span class="fw-semibold">${hostname||'无'}</span></div>
        <div class="kv"><span class="text-muted">CPU 核心</span><span class="fw-semibold">${cores}</span></div>
        <div class="kv"><span class="text-muted">总内存</span><span class="fw-semibold">${memGb} GB</span></div>
        <div class="kv"><span class="text-muted">运行时间</span><span class="fw-semibold">${up.toFixed(1)} 天</span></div>`;

      // 分区明细
      const parts = (sv.disk?.partitions||[]);
      let tbody = '';
      if (!parts.length){
        tbody = '<tr><td colspan="4" class="text-center text-muted">暂无磁盘分区数据</td></tr>'
      } else {
        parts.forEach(p=>{
          const u = Number(p.usage_percent||0);
          tbody += `<tr>
            <td><code>${p.mountpoint||p.device||'-'}</code></td>
            <td>${(p.device||'')}</td>
            <td style="width:40%"><div class="progress"><div class="progress-bar ${colorBy(u)}" style="width:${u}%">${pct(u)}</div></div></td>
            <td>${(p.fs_type||'')}</td>
          </tr>`;
        })
      }
      document.getElementById('disk-tbody').innerHTML = tbody;
    }
  </script>
</head>
<body>
  <nav class="navbar navbar-expand-lg navbar-dark">
    <div class="container-fluid">
      <a class="navbar-brand" href="/"><i class="bi bi-speedometer2 me-2"></i>AI-Ops 智能运维系统</a>
      <div class="navbar-nav ms-auto">
        <a class="nav-link" href="/">仪表板</a>
        <a class="nav-link" href="/log-types">日志类型</a>
        <a class="nav-link" href="/server-resources">服务器资源</a>
        <a class="nav-link" href="/reports">报告</a>
      </div>
    </div>
  </nav>
  ...
  <div class="container-fluid py-4">
    <div class="row mb-4">
      <div class="col">
        <div class="d-flex align-items-center">
          <div class="me-3"><i class="bi bi-hdd-network" style="font-size:2.2rem;color:#667eea;"></i></div>
          <div>
            <h1 class="h4 mb-1 fw-bold text-dark">服务器详情</h1>
            <div class="text-muted">实例：<span id="title-inst">-</span></div>
          </div>
        </div>
      </div>
      <div class="col-auto">
        <a class="btn btn-outline-primary me-2" href="/server-resources"><i class="bi bi-list-ul me-1"></i> 返回列表</a>
        <a class="btn btn-primary" href="/"><i class="bi bi-arrow-left me-1"></i> 返回首页</a>
      </div>
    </div>

    <div class="row g-3 mb-3" id="top-metrics"></div>

    <div class="row g-3">
      <div class="col-lg-7">
        <div class="main-card">
          <div class="card-header"><h5 class="card-title mb-0"><i class="bi bi-speedometer2 text-primary me-2"></i>资源使用情况</h5></div>
          <div class="card-body" id="bars">
      ...
      <div class="col-lg-5">
        <div class="main-card h-100">
          <div class="card-header"><h5 class="card-title mb-0"><i class="bi bi-info-circle text-primary me-2"></i>基本信息</h5></div>
          <div class="card-body">
            <div id="kv"></div>
          </div>
        </div>
    ...
    <div class="row g-3 mt-1">
      <div class="col-12">
        <div class="main-card">
          <div class="card-header"><h5 class="card-title mb-0"><i class="bi bi-device-hdd text-primary me-2"></i>磁盘分区</h5></div>
          <div class="card-body">
            <div class="table-responsive">
              <table class="table table-hover">
                <thead><tr><th>挂载点</th><th>设备</th><th>使用率</th><th>文件系统</th></tr></thead>
                <tbody id="disk-tbody"><tr><td colspan="4" class="text-center text-muted">加载中...</td></tr></tbody>
              </table>
            </div>
          </div>
        </div>
  ...
  <script>document.addEventListener('DOMContentLoaded', load);</script>
</body>
</html>
        """
    ...
    def log_types_page():
        """日志类型分析页面"""
        return FileResponse("static/log-types.html")
