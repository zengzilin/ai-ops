...
from config import SETTINGS, setup_logging, CACHE
from log_analyzer import LogAnalyzer
from app.services.prom_client import PrometheusClient, run_health_checks, run_comprehensive_inspection
from db import init_schema, insert_inspections, insert_inspection_summary
from inspection import InspectionEngine, InspectionScheduler, run_quick_inspection, run_full_inspection
...
def cmd_schedule(health_interval: int) -> None:
    """启动定时巡检"""
        ...
        def worker():
            while True:
                try:
                    ...
                    try:
                        from config import REDIS_CACHE as _RC
    ...
    # 启动调度器
    logger.info(f"巡检调度器已启动 - 巡检间隔: {health_interval}s")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_sch.add_argument("--health-interval", type=int, default=300, help="巡检间隔(秒)")
...
def main() -> None:
    ...
    try:
        ...
        if args.cmd == "setup":
            ...
            cmd_schedule(args.health_interval)
