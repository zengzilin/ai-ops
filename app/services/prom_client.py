...
def get_mock_server_resources() -> List[Dict[str, Any]]:
    """提供模拟服务器资源数据（当Prometheus不可用时）"""
    import random
    from datetime import datetime, timedelta
    
    mock_servers = [
        {"name": "prod-web-01", "ip": "192.168.1.101"},
        {"name": "prod-web-02", "ip": "192.168.1.102"},
        {"name": "prod-db-01", "ip": "192.168.1.201"},
        {"name": "prod-api-01", "ip": "192.168.1.301"},
        {"name": "prod-api-02", "ip": "192.168.1.302"},
    ]
    ...
    for server in mock_servers:
        ...
        load_5m = round(random.uniform(0.4, 2.8), 2)
        load_15m = round(random.uniform(0.3, 2.5), 2)
        
        resource = {
            "instance": f"{server['ip']}:9100",
            "cpu": {
            ...
            "memory": {
                ...
                "total_gb": random.choice([16, 32, 64]),
                "used_gb": round(random.uniform(5, 40), 1),
                "free_gb": round(random.uniform(2, 20), 1)
            },
            "disk": {
                "partitions": [
                    {
                        ...
                        "used_gb": round(random.uniform(30, 200), 1),
                        "free_gb": round(random.uniform(20, 100), 1)
                    },
                    {
                        "mountpoint": "/data",
                        "usage_percent": round(random.uniform(20, 70), 1),
                        "total_gb": random.choice([500, 1000, 2000]),
                        "used_gb": round(random.uniform(100, 800), 1),
                        "free_gb": round(random.uniform(100, 500), 1)
                    }
                ]
            },
            "network": {
            ...
            "system": {
                ...
                "last_reboot": (datetime.now() - timedelta(days=round(random.uniform(1, 60)))).isoformat()
            },
            "status": "ok",
            "timestamp": datetime.now().isoformat(),
            "mock_data": True  # 标识这是模拟数据
        }
        
        resources.append(resource)
    
    logger.info(f"生成了 {len(resources)} 个模拟服务器资源数据")
    return resources
...
def get_server_resources(prom: PrometheusClient) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"无法连接Prometheus服务器: {e}")
            logger.info("使用模拟服务器资源数据")
            return get_mock_server_resources()
        
        # 直接从node指标中获取实例，不依赖job名称过滤
        node_instances = []
        
        # 优先从node_load1指标获取实例（这个指标通常存在且唯一）
        try:
            load_instances = prom.get_metrics("node_load1")
            seen_instances = set()
                ...
                if instance_name and instance_name not in seen_instances:
                    ...
                    node_instances.append({
                        "metric": {
                            ...
                            "job": metric.get("job", "node-exporter")
                        }
                    })
            logger.info(f"从node_load1指标找到 {len(node_instances)} 个node实例")
        except Exception as e:
            logger.warning(f"获取node_load1指标失败: {e}")
        
        # 如果node_load1没有数据，尝试从CPU指标中获取
        if not node_instances:
            try:
                ...
                logger.info(f"从node_cpu_seconds_total指标找到 {len(node_instances)} 个node实例")
            except Exception as e:
                logger.warning(f"获取node_cpu_seconds_total指标失败: {e}")
